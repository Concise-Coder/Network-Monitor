...
class NetworkSpeedMonitor(tk.Tk):
    def __init__(self, update_interval=1000):
        ...
        self.hover_window.bind("<Button-3>", self.restore_gui)
    ...
    def bytes_to_megabits(self, bytes):
    ...
    def load_total_usage(self, current_sent, current_recv):
        if os.path.exists(TOTAL_USAGE_FILE):
            try:
                ...
                if total_upload > current_sent:
                    total_upload = current_sent
                if total_download > current_recv:
                    total_download = current_recv
    ...
    def save_total_usage(self):
        try:
            ...
            print(f"Error saving total usage data: {e}", file=sys.stderr)
    ...
    def update_speeds(self):
        ...
        sent_diff = current_bytes_sent - self.prev_bytes_sent
        recv_diff = current_bytes_recv - self.prev_bytes_recv
        ...
        self.total_upload += max(sent_diff, 0)
        self.total_download += max(recv_diff, 0)
